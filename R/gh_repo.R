...
gh_repo_list <- function(
	...
	args <- gh_args(
		c(
			...
			owner,
			"--limit",
			as.character(as.integer(limit)),
			"--json",
			paste(fields, collapse = ",")
		)
	)
	if (isTRUE(archived)) {
		args <- c(args, "--archived")
	}
	if (isTRUE(fork)) {
		args <- c(args, "--fork")
	}
	if (isTRUE(omit_archived)) {
		args <- c(args, "--no-archived")
	}
	if (isTRUE(omit_fork)) {
		args <- c(args, "--source")
	}
...
gh_repo_view <- function(
	...
	args <- c("repo", "view", shQuote(repository), "--json", paste(fields, collapse = ","))
